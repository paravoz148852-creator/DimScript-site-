...
async function initVK() {
    try {
        ...
        // Инициализируем VK
        VK.init({
            apiId: YOUR_APP_ID // Замените на ID вашего приложения
        });

        // Авторизация
        const auth = await new Promise(resolve => {
            VK.Auth.login(resolve, 2); // 2 - права доступа
        });

        if (auth.session) {
            vkUser = auth.session.user;
            console.log('VK user:', vkUser);
        }

    } catch (error) {
        console.log('VK API недоступен, работаем в демо-режиме');
    }
}

// Вызовите инициализацию при загрузке
